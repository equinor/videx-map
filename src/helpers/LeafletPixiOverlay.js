...
const pixiOverlayClass = {
  ...
  initialize: function (drawCallback, pixiContainer, options) {
    ...
    this._doubleBuffering = this.options.doubleBuffering;
  },
  ...
  setRenderer: async function () {
    // using WebGlRenderer directly instead of autoDetectRenderer,
    // as the latter's promise dies in the dockerized version
    // without resolving or rejecting
    this._renderer = new WebGLRenderer();
    await this._renderer.init(this._rendererOptions);

    if (this._doubleBuffering) {
      this._auxRenderer = new WebGLRenderer();
      await this._auxRenderer.init(this._rendererOptions);
    }
  },
  ...
  _update: function (e) {
    ...
    if (
      ...
      if (this._renderer.resolution !== window.devicePixelRatio) {
...
export const LeafletPixiOverlay = L.PixiOverlay;
