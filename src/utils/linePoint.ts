...
export function closestPointOnLine(
  ...
  const dir: Vector2 = Vector2.sub(point, lineStart).mutable.rotate(-lineAngle);

  // If outside
  if (dir[0] < 0) return lineStart;
  else if (dir[0] > len) return lineEnd;

  return lineDir.mutable.rescale(dir[0]).add(lineStart[0], lineStart[1])
    .immutable;
}
 ...
 * @param lineEnd End of line segment
 */
export function displacementToLineOrigin(
  point: VectorLike,
  ...
  return lineDir.set(point).sub(lineStart).rotate(-lineAngle).modify(Math.abs)
    .immutable;
}
