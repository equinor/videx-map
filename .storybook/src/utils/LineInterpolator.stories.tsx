...
export const Circle = () => {
  {
    ...
    for (let i = 0; i <= 260; i += 20) {
      points.push(Vector2.right.rotateDeg(i).rescale(200).add(center));
    }

    // Create the line interpolator
    const interp: LineInterpolator = new LineInterpolator(points, 0.001);

    const svg = create('svg').attr('width', '500px').attr('height', '500px');
...
export const Ticks = () => {
  {
    ...
    for (let i = 20; i <= 780; i += 40) {
      points.push(new Vector2(i, 100 + 30 * Math.sin(i * 0.02)));
    }
    ...
    for (let i = 0.1; i <= 0.9; i += 0.1) {
      ...
      svg
        ...
        .attr(
          'points',
          `${ll.x},${ll.y} ${lr.x},${lr.y} ${ur.x},${ur.y} ${ul.x},${ul.y}`,
        )
        .attr('fill', 'White')
        .attr('fill', 'OliveDrab');
    }
