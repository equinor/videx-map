...
import { autoDetectRenderer, VERSION, isWebGLSupported } from 'pixi.js';
import L from 'leaflet';

(function (factory) {
  factory(L, { autoDetectRenderer, VERSION, isWebGLSupported });
})(function (L, { autoDetectRenderer, VERSION, isWebGLSupported }) {
  ...
  function setEventSystem(
    ...
    const eventSystem =
      VERSION < '7' ? renderer.plugins.interaction : renderer.events;
  ...
  if (L.version >= '1') {
    ...
    L.Map.prototype.getZoomScale = function (toZoom, fromZoom) {
      ...
      const newZoom = fromZoom ?? this._zoom;
      return crs.scale(toZoom) / crs.scale(newZoom);
    };
